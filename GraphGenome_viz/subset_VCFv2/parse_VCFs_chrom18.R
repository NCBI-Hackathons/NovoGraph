...
chrom18 = readVcf('chr18_only.vcf') 

fixed_chrom18 = fixed(chrom18)

fixed_chrom18$QUAL = NULL

fixed_chrom18$FILTER = NULL

fixed_chrom18$ALT = CharacterList(fixed_chrom18$ALT)

dt = as.data.table(fixed_chrom18)
...
plt1 = ggplot(insdt, aes(x=insertions)) + geom_histogram(binwidth=1, col="#46ACC8") + scale_x_continuous(limits = c(1, 50)) + ylim(0, 50010) + 
    labs(title="Chrom18, Insertion lengths <= 50, binwidth=1") + labs(y="Count", x="Insertion Lengths")
...
plt2 = ggplot(deldt, aes(x=deletions)) + geom_histogram(binwidth=1, col="#46ACC8") + scale_x_continuous(limits = c(1, 50)) + ylim(0, 50010) + 
    labs(title="Chrom18, Deletions lengths <= 50, binwidth=1") + labs(y="Count", x="Deletion Lengths")  
...
plt3 = ggplot(insdt, aes(x=insertions)) + geom_histogram(binwidth=5, col="#46ACC8") + scale_x_continuous(limits = c(51, 229236)) +
    ...
    labs(title="Chrom18, Insertion lengths > 50, binwidth=5") + labs(y="Count", x="Insertion Lengths")
...
plt4 = ggplot(deldt, aes(x=deletions)) + geom_histogram(binwidth=5, col="#46ACC8") + scale_x_continuous(limits = c(51, 229236)) +
    scale_y_continuous(limits = c(0, 650)) +      
    labs(title="Chrom18, Deletion lengths > 50, binwidth=5") + labs(y="Count", x="Deletion Lengths")

ppdf(print(plt4))


fwrite(insdt, file = "chrom18_insertions.csv", sep = ",")

fwrite(deldt, file = "chrom18_deletions.csv", sep = ",")  
